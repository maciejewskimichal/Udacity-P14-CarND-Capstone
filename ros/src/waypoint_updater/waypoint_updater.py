...
class WaypointUpdater(object):
    ...
    def decelerate_waypoints(self, waypoints, closest_wp_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            stop_idx = max(self.stopline_wp_idx - closest_wp_idx - 2, 0) # Two waypoints back from line so front of car stops on line
...
# simple controler - run loop_waypoints   
    def loop_waypoints(self):
        rate = rospy.Rate(50)
        while not rospy.is_shutdown():
            if self.pose and self.base_waypoints:
                #Get closest waypoint
                closest_waypoint_idx = self.get_closest_waypoint_idx()
                self.publish_waypoints_close(closest_waypoint_idx)
            rate.sleep()
 
    def publish_waypoints_close(self, closest_idx):
        lane           = Lane()
        lane.header    = self.base_waypoints.header
